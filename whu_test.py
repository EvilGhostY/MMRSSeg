...
def label2rgb(mask):
    ...
    mask_rgb[np.all(mask_convert == 0, axis=0)] = [0, 102, 204]
    ...
    mask_rgb[np.all(mask_convert == 4, axis=0)] = [0, 167, 85]
    mask_rgb[np.all(mask_convert == 5, axis=0)] = [255, 255, 93]
    mask_rgb[np.all(mask_convert == 6, axis=0)] = [153, 102, 152]
...
def get_args():
    ...
    arg("-c", "--config_path", type=Path, default=r'config/potsdam/csfaformer.py', help="Path to config")
    arg("-o", "--output_path", type=Path, default=r'fig_results/potsdam/csfaformer', help="Path where to save resulting masks.")
