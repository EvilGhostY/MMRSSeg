...
def parse_args():
    ...
    parser.add_argument("--img-dir", default="data\\potsdam\\test_irrg")
    parser.add_argument("--dsm-dir", default="data\\potsdam\\test_dsms")
    parser.add_argument("--mask-dir", default="data\\potsdam\\test_masks_eroded")
    parser.add_argument("--output-img-dir", default="data\\potsdam\\test\\irs")
    parser.add_argument("--output-dsm-dir", default="data\\potsdam\\test\\dsms")
    parser.add_argument("--output-mask-dir", default="data\\potsdam\\test\\masks")
    parser.add_argument("--eroded", action='store_true',default=True)
...
def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--img-dir", default="data\\potsdam\\train_irrg")
    parser.add_argument("--dsm-dir", default="data\\potsdam\\train_dsms")
    parser.add_argument("--mask-dir", default="data\\potsdam\\train_masks")
    parser.add_argument("--output-img-dir", default="data\\potsdam\\train\\irs")
    parser.add_argument("--output-dsm-dir", default="data\\potsdam\\train\\dsms")
    parser.add_argument("--output-mask-dir", default="data\\potsdam\\train\\masks")
...
def get_img_mask_padded(image, dsm_n, mask, patch_size, mode):
    img, dsm,  mask = np.array(image), np.array(dsm_n), np.array(mask)
    ...
    img_pad, dsm_pad, mask_pad = pad_img['image'], pad_dsm['image'], pad_mask['image']
...
#    dsm_pad = cv2.cvtColor(np.array(dsm_pad), cv2.COLOR_RGB2BGR)
    ...
    return img_pad, dsm_pad, mask_pad
...
def image_augment(image, dsm, mask, patch_size, mode='train', val_scale=1.0):
    image_list = []
    dsm_list = []
    mask_list = []
    ...
    if mode == 'train':
        ...
        image_h_vlip, dsm_h_vlip, mask_h_vlip = h_vlip(image.copy()), h_vlip(dsm.copy()), h_vlip(mask.copy())
        image_v_vlip, dsm_v_vlip, mask_v_vlip = v_vlip(image.copy()), v_vlip(dsm.copy()), v_vlip(mask.copy())
        ...
        for i in range(len(image_list_train)):
            image_tmp, dsm_tmp, mask_tmp = get_img_mask_padded(image_list_train[i],dsm_list_train[i], mask_list_train[i], patch_size, mode)
        ...
        image, dsm, mask = rescale(image.copy()), rescale(dsm.copy()), rescale(mask.copy())
        image, dsm, mask = get_img_mask_padded(image.copy(), dsm.copy(), mask.copy(), patch_size, mode)
...
def patch_format(inp):
    (img_path, dsm_path, mask_path, imgs_output_dir, dsms_output_dir, masks_output_dir, eroded, gt, rgb_image,
    ...
    image_list, dsm_list, mask_list = image_augment(image=img.copy(),dsm=dsm.copy(), mask=mask.copy(), patch_size=split_size,
    ...
    for m in range(len(image_list)):
        ...
        for y in range(0, img.shape[0], stride):
            for x in range(0, img.shape[1], stride):
                ...
                img_tile, dsm_tile, mask_tile = img_tile_cut, dsm_tile_cut, mask_tile_cut
...
if __name__ == "__main__":
    ...
    dsm_paths_raw = glob.glob(os.path.join(dsms_dir, "*.jpg"))
    dsm_paths = [p[:-8] for p in dsm_paths_raw]
    ...
    inp = [(img_path, dsm_path, mask_path, imgs_output_dir, dsms_output_dir, masks_output_dir, eroded, gt, rgb_image,
