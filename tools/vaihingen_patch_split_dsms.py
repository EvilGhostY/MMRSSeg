...
def parse_args():
    ...
    parser.add_argument("--dsm-dir", default="data/vaihingen/train_dsm")  # 新增DSM输入目录
    ...
    parser.add_argument("--output-dsm-dir", default="data/vaihingen/train/dsms")  # 新增DSM输出目录
...
def get_img_mask_padded(image, mask, dsm, patch_size, mode):
    img, mask, dsm = np.array(image), np.array(mask), np.array(dsm)
    ...
    pad_dsm = albu.PadIfNeeded(min_height=h, min_width=w, position='bottom_right',
                               border_mode=0, value=0)(image=dsm)
    img_pad, mask_pad, dsm_pad = pad_img['image'], pad_mask['image'], pad_dsm['image']
    img_pad = cv2.cvtColor(np.array(img_pad), cv2.COLOR_RGB2BGR)
    mask_pad = cv2.cvtColor(np.array(mask_pad), cv2.COLOR_RGB2BGR)

    return img_pad, mask_pad, dsm_pad
...
def image_augment(image, mask, dsm, patch_size, mode='train', val_scale=1.0):
    image_list = []
    mask_list = []
    dsm_list = []
    ...
    if mode == 'train':
        ...
        image_h_vlip, mask_h_vlip, dsm_h_vlip = h_vlip(image.copy()), h_vlip(mask.copy()), h_vlip(dsm.copy())
        image_v_vlip, mask_v_vlip, dsm_v_vlip = v_vlip(image.copy()), v_vlip(mask.copy()), v_vlip(dsm.copy())
        ...
        for i in range(len(image_list_train)):
            image_tmp, mask_tmp, dsm_tmp = get_img_mask_padded(image_list_train[i], mask_list_train[i],
                                                               dsm_list_train[i], patch_size, mode)
        ...
        image, mask, dsm = rescale(image.copy()), rescale(mask.copy()), rescale(dsm.copy())
        image, mask, dsm = get_img_mask_padded(image.copy(), mask.copy(), dsm.copy(), patch_size, mode)
        mask = rgb_to_2D_label(mask.copy())
        image_list.append(image)
        mask_list.append(mask)
        dsm_list.append(dsm)

    return image_list, mask_list, dsm_list


def randomsizedcrop(image, mask, dsm):
    h, w = image.shape[0], image.shape[1]
    crop = albu.RandomSizedCrop(min_max_height=(int(3*h//8), int(h//2)), width=h, height=w)(image=image.copy(), mask=mask.copy(), dsm=dsm.copy())
    img_crop, mask_crop, dsm_crop = crop['image'], crop['mask'], crop['dsm']
    return img_crop, mask_crop, dsm_crop


def car_aug(image, mask, dsm):
    assert image.shape[:2] == mask.shape == dsm.shape
    v_flip = albu.VerticalFlip(p=1.0)(image=image.copy(), mask=mask.copy(), dsm=dsm.copy())
    h_flip = albu.HorizontalFlip(p=1.0)(image=image.copy(), mask=mask.copy(), dsm=dsm.copy())
    rotate_90 = albu.RandomRotate90(p=1.0)(image=image.copy(), mask=mask.copy(), dsm=dsm.copy())
    # blur = albu.GaussianBlur(p=1.0)(image=image.copy())
    image_vflip, mask_vflip, dsm_vflip = v_flip['image'], v_flip['mask'], v_flip['dsm']
    image_hflip, mask_hflip, dsm_hflip = h_flip['image'], h_flip['mask'], h_flip['dsm']
    image_rotate, mask_rotate, dsm_rotate = rotate_90['image'], rotate_90['mask'], rotate_90['dsm']
    ...
    dsm_list = [dsm, dsm_vflip, dsm_hflip, dsm_rotate]

    return image_list, mask_list, dsm_list

def vaihingen_format(inp):
    (img_path, mask_path, dsm_path, imgs_output_dir, masks_output_dir, dsms_output_dir, eroded, gt, mode, val_scale,
     split_size, stride) = inp
        ...
        for y in range(0, img.shape[0], stride):
            for x in range(0, img.shape[1], stride):
                ...
                if img_tile.shape[0] == split_size and img_tile.shape[1] == split_size \
                    ...
                    image_crop, mask_crop, dsm_crop = randomsizedcrop(img_tile, mask_tile, dsm_tile)
                    ...
                    if cf[4] > 0.1 and mode == 'train':
                        car_imgs, car_masks, car_dsms = car_aug(image_crop, mask_crop, dsm_crop)
                        for i in range(len(car_imgs)):
                            ...
                            out_dsm_path = os.path.join(dsms_output_dir,
                                                        "{}_{}_{}_{}.tif".format(dsm_filename, m, k, i))
