...
class WaveletGate_DownModule(nn.Module):
    def __init__(self, in_channel=64, ratio=4):
        super(WaveletGate_DownModule, self).__init__()

        self.wavegate = Channel_Attention_Module_FC(in_channel, ratio)
        self.LL = ConvBNReLU(in_channel, in_channel, kernel_size=1)
        self.HL = ConvBNReLUx(in_channel, in_channel, kernel_size=3)
        self.LH = ConvBNReLUy(in_channel, in_channel, kernel_size=3)
        self.HH = ConvBNReLU(in_channel, in_channel, kernel_size=3)

    def forward(self, x):
        x_LL, x_HL, x_LH, x_HH = dwt_init(x)
        x_weight = self.wavegate(x)
        out = torch.cat((x_weight * self.LL(x_LL),x_weight * self.HL(x_HL),x_weight * self.LH(x_LH),x_weight * self.HH(x_HH)),dim=1)
...
class PatchMerging_DWT(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        ...
        self.dim = dim
#        self.reduction = nn.Linear(4 * dim, 2 * dim, bias=False)
        self.dwtdown = WaveletGate_DownModule(dim , ratio=8)

        self.norm = norm_layer(4 * dim)
        self.reduction = Conv(4 * dim, 2 * dim, kernel_size=3,bias=False)
    ...
    def forward(self, x, H, W):
        ...
        x = self.dwtdown(x.permute(0,3,1,2))   # B 4 * C H/2 W/2
...
def swinTransformer(pretrained=True, freeze_stages=-1,embed_dim=64,depths=(2, 2, 6, 2),num_heads=(4, 8, 16, 32),
                  weight_path='GeoSeg\pretrain_weights/stseg_small.pth'):
...
if __name__ == "__main__":
    net = swinTransformer(pretrained=True, freeze_stages=-1,
                  weight_path='pretrain_weights/stseg_base.pth')
    net.cuda()
    net.train()
    in_ten = torch.randn(4, 64, 256, 256).cuda()
    out1,out2,out3,out4 = net(in_ten)
    print(out1.shape)
    print(out2.shape)
    print(out3.shape)
    print(out4.shape)